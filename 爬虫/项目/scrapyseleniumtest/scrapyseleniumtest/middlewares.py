...
class SeleniumMiddleware(object):
    ...
    def __init__(self,timeout=None):
        ...
        self.browser.set_window_size(1300,700)
    ...
    def process_request(self, request, spider):
        ...
        :param spider:
        :return:
        为什么没有看到 页面下拉呢，，？？以后 下载的时候，顺便把文章一键三联
        流程：  根据输入框，模拟输入，点击 确认按钮，进行翻页，
        '''
        ...
        try:
            ...
            if page > 1:
                input = self.wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '#J_bottomPage > span.p-skip '
                                                                                         '> input'))) 
                input.clear()
                ...
                # 等待 #J_goodsList 加载出来，为页面数据，加载出来之后，在返回网页源代码  response
                self.wait.until(EC.text_to_be_present_in_element((By.CSS_SELECTOR, '#J_bottomPage > span.p-num > a.curr'),str(page)))
...
class CheckUserAgent:
    def process_response(self,request,spider,response):
        print(response.header['User-Agent'])

        #  返回给 response  ,交给 爬虫spider,,返回qureest，交给 调度器
